...
async function restoreBackupLive(e){
  ...
  try{
    ...
    for(const c of data.cases){if(!c?.doc_code||!c?.claim_no||!c?.company)throw new Error('Every backup case must contain doc_code, company and claim_no.');if(docs.has(c.doc_code))throw new Error(`Duplicate document code in backup: ${c.doc_code}`);docs.add(c.doc_code);}
