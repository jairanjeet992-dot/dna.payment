...
async function run() {
  ...
  err = await client.from('case_ownership_transfers').delete().neq('id', -1);
  console.log("Transfers:", err.error);
  
  err = await client.from('investigator_audit_log').delete().neq('action', '__never_matches__');
  console.log("Audit log:", err.error);
  
  err = await client.from('investigator_documents').delete().neq('document_name', '__never_matches__');
